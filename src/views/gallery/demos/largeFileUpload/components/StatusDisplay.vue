	...
	<div class="status-display">
      ...
      <span v-if="!hashComplete" class="hash-calculating">
        (特征码计算中: {{ preparingProgress }}%)
			...
			<span class="upload-speed">当前速度：{{ speed }}</span>
			<span class="upload-time">剩余时间：{{ remainingTime }}</span>
		</template>
...
<script setup>
	import { Upload, CircleCheckFilled, CircleCloseFilled, VideoPause } from '@element-plus/icons-vue'
	import { formatSize } from '../utils/fileUtils.js'

	defineProps({
		status: { type: String, required: true },
		speed: { type: String, default: '计算中...' },
		remainingTime: { type: String, default: '计算中...' },
